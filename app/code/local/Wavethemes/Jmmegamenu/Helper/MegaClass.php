...
class Wavethemes_Jmmegamenu_Helper_MegaClass extends Mage_Core_Helper_Abstract {
        ...
        foreach ($collections as $item) {
            ...
            if (is_array($blocks) && count($blocks) > 0) {
                ...
                for ($col = 0; $col < $cols; $col++) {
                    ...
                    if ($cols > 1)
                        $item->content .= $this->beginSubMenuModules($item, 1, $pos, $col, true);
                    $i = $col;
                    while ($i < $total) {
                        $block = $blocks[$i];
                        $i += $cols;
        ...
        if (isset($this->children[$pid]) && $this->children[$pid]) {
            ...
            for ($col = 0, $j = 0; $col < $cols && $j < $total; $col++) {
                ...
                while ($i < $tmp->col[$col] && $j < $total) {
                    ...
                    } else if ($level < $this->params->end_level) {
    ...
    public function beginSubMenuModules($item, $level = 0, $pos, $i, $return = false) {
        $data = '';
        ...
        if ($level) {
            ...
            if ($item->megaparam->get('group')) {
                ...
                if (!$colw)
                    $colw = $this->items[$item->parent]->megaparam->colw ? $this->items[$item->parent]->megaparam->colw : 200;
    ...
    public function endSubMenuModules($item, $level = 0, $return = false) {
        $data = '';
        if ($level) {
            if ($item->megaparam->get('group')) {
    ...
    public function beginSubMenuItems($pid = 0, $level = 0, $pos, $i, $return = false) {
        ...
        if (@$this->children[$pid])
            $data .= "<ul class=\"megamenu level$level\">";
        if ($return)
            return $data;
        else
            echo $data;
    ...
    public function beginMenuItem($mitem = null, $level = 0, $pos = '') {
    ...
    public function endMenuItem($mitem = null, $level = 0, $pos = '') {
        if ($mitem->megaparam->group)
            echo "</div>";
        echo "</li>";
    }
    ...
    public function beginMenu() {
        ...
        $menuGroupId = $this->params->menu_group_id;
        $additionClass = $this->params->addition_class;

        echo "<div id='jm-megamenu-{$menuGroupId}' class='{$additionClass}'>\n";
        echo "<div class=\"{$animation} jm-megamenu clearfix\" >\n";
    }
    ...
    public function genClass($mitem, $level, $pos) {
        $cls = "mega" . ($pos ? " $pos" : "");
    ...
    public function genMenuItem($item, $level = 0, $pos = '', $ret = 0) {
        $data = '';
        $tmp = $item;
        $tmpname = ($this->params->is_mega_menu && !$tmp->showtitle) ? '' : $tmp->title;
...
//                if ($item->getData('catid')) {
//                    $category = Mage::getModel('catalog/category')->load($item->getData('catid'));
//                } else {
//                    $category = Mage::helper('jmmegamenu')->getCategoryByUrl($item->getData('category'));
//                }
...
//                $tmpname .= '<span class="shownumproduct"> (' . $collection->getSize() . ')</span>';
                $category = Mage::helper('jmmegamenu')->getCategoryByUrl($item->getData('category'));
                $tmpname .= '<span class="shownumproduct"> (' . $category->getProductCount(). ')</span>';
            }
        }
        ...
        if (isset($tmp->megaparams->desc) && $tmp->megaparams->desc) {
            $txt .= '<span class="menu-desc">' . Mage::helper('jmmegamenu')->_($tmp->megaparams->desc) . '</span>';
        }

...
class Wavethemes_Jmmegamenu_Block_Adminhtml_Jmmegamenugroup_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	...
	{
		$storeOptions = array();
		foreach(Mage::getModel('core/store')->getCollection()->toOptionArray() as $option){
			$storeOptions[$option['value']] = $option['label'];
		}
		...
		$this->addColumn('menutype', array(
			'header' => Mage::helper('jmmegamenu')->__('Menu Key'),
			'align' => 'left',
			'index' => 'menutype',
		));
		if($storeOptions){
			$this->addColumn('storeid', array(
				'header' => Mage::helper('jmmegamenu')->__('Store'),
				'align' => 'left',
				'index' => 'storeid',
				'type' => 'options',

...
class Wavethemes_Jmmegamenu_Block_Adminhtml_Jmmegamenugroup_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
	...
	protected function _prepareForm()
	{
		...
		$this->setForm($form);
		$fieldset = $form->addFieldset('jmmegamenugroup_form', array('legend' => Mage::helper('jmmegamenu')->__('Menu group information')));
		...
		$fieldset->addField('menutype', 'text', array(
			'label' => Mage::helper('jmmegamenu')->__('Menu key'),
			'class' => 'required-entry',
			'required' => true,
			'name' => 'menutype',
		));
		...
		$fieldset->addField('description', 'textarea', array(
			'label' => Mage::helper('jmmegamenu')->__('Menu Description'),
			'name' => 'description',
		));

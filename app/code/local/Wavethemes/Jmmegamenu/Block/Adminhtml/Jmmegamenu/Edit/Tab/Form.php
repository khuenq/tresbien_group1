...
class Wavethemes_Jmmegamenu_Block_Adminhtml_Jmmegamenu_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
	...
	protected function _prepareForm()
	{
        ...
        $menuGroup = Mage::registry('jmmegamenugroup_data');
        if ($menuGroup){
            $store = Mage::app()->getStore($menuGroup->getData('storeid'));
        } else {
		...
		$this->setForm($form);
		$fieldset = $form->addFieldset('jmmegamenu_form', array('legend' => Mage::helper('jmmegamenu')->__('Menu item information')));
		...
		$fieldset->addField('browserNav', 'select', array(
			...
			'values' => array(
				...
				array('value' => '2', 'label' => 'New window without navigation'),
			),
		));
